...
for (ix.seed in seq(params$seed, params$seed + params$seed.additional)) {
  ...
  for (ix.alpha in params$alpha) {
    aux.call <- function(xdata, ydata, network, alpha, seed) { my.cv.glmnet(alpha, xdata, ydata, network, n.cores = params$n.cores, seed = seed) }
    
    cv.classic <- run.cache(aux.call, xdata, ydata, rep(1, ncol(xdata)), ix.alpha, ix.seed, 
                            cache.prefix = 'cv.glmSparseNet.elastic.net', cache.digest = list(xdata.digest.cache))
    ...
    cv.hub     <- run.cache(aux.call, xdata, ydata, penalty.factor.degree.new, ix.alpha, ix.seed, 
                            cache.prefix = 'cv.glmSparseNet.hub', cache.digest = list(xdata.digest.cache))
    ...
    cv.orphan  <- run.cache(aux.call, xdata, ydata, penalty.factor.orphan, ix.alpha, ix.seed, 
                            cache.prefix = 'cv.glmSparseNet.orphan', cache.digest = list(xdata.digest.cache))
    ...
    save(new.line, file = sprintf('cv.value_%s_xdata-%s_ydata-%s', loose.rock::digest.cache(new.line), xdata.digest.cache, loose.rock::digest.cache(ydata)))
...
```{r, echo=FALSE}
km.all <- apply(alpha.models, 1:2, function(ix.name) {
...
km.all %>% as.data.frame %>%  group_by(alpha) %>% summarise_all(list(mean = mean, SE = sd)) %>% knitr::kable()
```
...
```{r, echo=FALSE}
c.index.all <- apply(alpha.models, 1:2, function(ix.name) {
  if (is.numeric(ix.name[[1]])) {
    return(ix.name[[1]])
  } else {
    best.coef <- coef(ix.name[[1]], s = 'lambda.min')[,1]
...
c.index.all %>% as.data.frame %>%  group_by(alpha) %>% summarise_all(mean) %>% knitr::kable()
```
