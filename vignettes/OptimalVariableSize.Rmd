...
rmarkdown::render('OptimalVariableSize.Rmd')
```
...
for (ix.seed in seq(params$seed, params$seed + params$seed.additional)) {
  ...
  for (ix.alpha in params$alpha) {
    ...
    save(new.line, file = sprintf('cv.value_%s_xdata-%s_ydata-%s', glmSparseNet::digest.cache(new.line), xdata.digest.cache, loose.rock::digest.cache(ydata)))
