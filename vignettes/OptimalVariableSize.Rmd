...
params:
  project: 'skcm.data.2018.09.11' # skcm prad brca
...
.Last.value <- flog.appender(appender.tee(sprintf('logger-optimal-%s.txt', format(Sys.time(), '%Y%m%d-%Hh%Mm%S'))))
...
cat('prepare.tcga.survival.data.old(\'', params$project, '\',\n', sep = '')
cat('                               \'', params$tissue, '\',\n', sep = '')
...
my.data <- run.cache(prepare.tcga.survival.data, 
                     ...
                     #subtract.suv.column       = 'days_to_submitted_specimen_dx',
...
if (params$degree.type == 'oldie') {
  ...
  degree <- run.cache(string.db.homo.sapiens, cache.prefix = "stringDB") %>% {
...
km.all <- apply(alpha.models[,-2], 1:2, function(ix.name) {
  if (is.numeric(ix.name[[1]])) {
    ...
    return(separate2groups.cox(best.coef, xdata, ydata)$pvalue)
  }
})
