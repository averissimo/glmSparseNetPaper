...
alpha.models <- tibble(alpha = numeric(), glmnet = list(), hub = list(), orphan = list())

for (ix.seed in seq(params$seed, params$seed + 10)) {
  ...
  for (ix.alpha in params$alpha) {
    ...
    alpha.models <- rbind(alpha.models, tibble(alpha  = ix.alpha, 
                                               glmnet = list(cv.classic$model), 
                                               hub    = list(cv.degree$model), 
...
km.all %>% as.data.frame %>%  group_by(alpha) %>% summarise_all(mean) %>% knitr::kable()
```
