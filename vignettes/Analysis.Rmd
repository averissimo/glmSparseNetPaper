...
```{r load_tcga, echo=FALSE, include=FALSE}
...
```{r survival_curves, fig.asp=1, echo=FALSE}
...
```{r test.clinical.variables, fig.asp=.8, echo=FALSE, eval=FALSE}
...
```{r plots_x_no_scale, echo=FALSE, fig.asp=1}
...
#### LOG10 Scale on _X_-axis {-}

```{r plots_x_scale, echo=FALSE, fig.asp=1}
transf.d <- reshape2::melt(my.df[,c('ix', 
                                     'original', 
                                     'penalty.factor.degree.old',
                                     'penalty.factor.degree.log',
...
```{r risk.test, fig.asp=1, echo=FALSE}
show.relative.risk.distribution(fit.df, 'Test')
```

### Train Set {-}

```{r risk.train, fig.asp=1, echo=FALSE}
show.relative.risk.distribution(fit.df, 'Train')
```

## Kaplan-Meier Curves

```{r plot.km, echo=FALSE, fig.asp=.6}
my.km.train <- list()
my.km.test <- list()
ix <- 1
for (ix.name in names(model.results$coefs)) {
...
```{r venn.coef, echo=FALSE, fig.asp=.6, results='asis'}
...
```{r hallmarks, fig.asp=1, echo=FALSE}
...
```{r biclust, fig.asp=1, echo=FALSE, eval=FALSE}
#*(not running at the moment)
...
```{r c.index.iter.dist, echo=FALSE, fig.asp=.6}
big.df %>%
...
```{r rank.c.index.test, echo=FALSE}
...
```{r en_orphan, echo=FALSE, fig.asp=.4}
filter.by <- 'C-Index (Test set)'
...
```{r hub_orphan, echo=FALSE, fig.asp=.4}
filter.by <- 'C-Index (Test set)'
types     <- c('Orphan', 'Hub')
...
```{r c.iter.train, echo=FALSE, fig.asp=.6}
big.df %>%
  mutate(base.model = prettify.labels(model, 'string.base'),
         string.model = prettify.labels(model, 'string.short'),
         model.name = prettify.labels(model, 'model')) %>%
...
```{r rank.c.index.train, echo=FALSE}
...
```{r rank.log.rank.train, echo=FALSE}
...
```{r en_hub_logrank, echo=FALSE, fig.asp=.4}
filter.by <- 'Log-rank (Test set)'
types     <- c('Elastic Net', 'Hub')
...
```{r en_orphan_logrank, echo=FALSE, fig.asp=.4}
filter.by <- 'Log-rank (Test set)'
types     <- c('Elastic Net', 'Orphan')
...
```{r hub_orphan_logrank, echo=FALSE, fig.asp=.4}
filter.by <- 'Log-rank (Test set)'
types     <- c('Orphan', 'Hub')
...
```{r consensus.count, echo=FALSE, fig.asp=.35}
melt(all.conse.tbl) %>%
...
```{r consensus.venn, results='asis', echo=FALSE, fig.asp=.8, out.width='40%'}
all.names  <- names(model.results$coefs)
all.names  <- all.names[grep('^(degree_new--)|^(orphan--)|^(glmnet--)', all.names)]
all.model  <- unique(gsub('^([a-zAZ]+)--(.+)$', '\\1', gsub('_new', '', all.names)))
all.origin <- gsub('^([a-zAZ]+)--(.+)$', '\\2', gsub('_new', '', all.names))
...
```{r consensus.count.pvalue, echo=FALSE, fig.asp=.35, results='asis'}
...
```{r consensus.venn.pvalue, echo=FALSE, fig.asp=.8, out.width='40%'}
if(length(all.conse.pvalue.tbl) == 0) {
  flog.warn('None of the models was statistical significant.')
} else {
  all.names  <- names(model.results$coefs)
...
### Best Elastic Net model _(C-Index test)_ {- .tabset .tabset-fade .tabset-pills}

```{r best.en.c.index, echo=FALSE, fig.asp=1, results='asis'}
filter.model <- 'Elastic Net'
best.model.print(filter.model, best.ones, xdata, ydata.month, ntimes.results)
```

### Best Hub model _(C-Index test)_ {- .tabset .tabset-fade .tabset-pills}

```{r best.hub.c.index, echo=FALSE, fig.asp=1, results='asis'}
filter.model <- 'Hub'
best.model.print(filter.model, best.ones, xdata, ydata.month, ntimes.results)
```

### Best Orphan model _(C-Index test)_ {- .tabset .tabset-fade .tabset-pills}

```{r best.orphan.c.index, echo=FALSE, fig.asp=1, results='asis'}
filter.model <- 'Orphan'
best.model.print(filter.model, best.ones, xdata, ydata.month, ntimes.results)
```
...
### Best Elastic net _(Log-rank test)_ {- .tabset .tabset-fade .tabset-pills}

```{r best.en.log.rank, echo=FALSE, fig.asp=1, results='asis'}
filter.model <- 'Elastic Net'
best.model.print(filter.model, best.ones, xdata, ydata.month, ntimes.results)
```

### Best Hub model _(Log-rank test)_ {- .tabset .tabset-fade .tabset-pills}

```{r best_hub.log.rank, echo=FALSE, fig.asp=1, results='asis'}
filter.model <- 'Hub'
best.model.print(filter.model, best.ones, xdata, ydata.month, ntimes.results)
```

### Best Orphan model _(Log-rank test)_ {- .tabset .tabset-fade .tabset-pills}

```{r best_orphan.log.rank, echo=FALSE, fig.asp=1, results='asis'}
filter.model <- 'Orphan'
