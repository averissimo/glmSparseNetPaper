...
params:
  ...
  ntimes: !r 100  # number of times that training set is tested
  #
  seed: !r 1985   # seed being used throughout the cde
  balanced.sets: !r FALSE
...
```{r survival_curves, fig.height=10}
...
if (params$train >= 1) {
  ...
  multiple.runs <- run.cache(build.multiple.runs,
                             params, xdata, ydata,
                             penalty.factor.degree.new, 
                             penalty.factor.degree.old, 
                             penalty.factor.orphan,
  ...
  ntimes.results <- multiple.runs$ntimes.results
