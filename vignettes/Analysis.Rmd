...
params:
  ...
  ntimes: !r 1000 # number of times that training set is tested
  #
  seed: !r 1985
  n.cores: !r 10
  #
...
* *Alpha / Target variables*: `r paste(sapply(names(params$target.vars), function(ix) {sprintf('%s: %.2f / %d', ix, params$target.vars[[ix]]$alpha, params$target.vars[[ix]]$vars)}), collapse = ', ')`
...
rmarkdown::render('Analysis.Rmd', output_file = 'skcm-center.html', 
                  params = list(normalization = 'center', project = 'skcm', tissue = 'metastatic'), output_format = BiocStyle::html_document())
rmarkdown::render('Analysis.Rmd', output_file = 'skcm-max.html', 
                  params = list(normalization = 'max', project = 'skcm', tissue = 'metastatic'), output_format = BiocStyle::html_document())
```
...
* *Tissue type*: **`r params$tissue`**
    * `r if(params$coding.genes) {'Only using coding genes'} else {'Using all genes (coding and non-coding)'}`
* *Degree calculation*: **`r params$degree.type`**
* *Subtract surivival time*: **`r params$subtract.surv.column`**
...
calc.params <- params[c('subset', 'train', 'calc.params.old', 'seed', 'target.vars')]
...
for(ix in c('Train', 'Test')) {
  print(
    fit.df %>%
      ...
      mutate(type.model = prettify.labels(type, ret.value = 'model')) %>%
      filter(set == ix) %>%
      ggplot() +
        #geom_freqpoly(aes(relative.risk, color = set), bins = 150) +
        stat_density(aes(relative.risk, fill = type.model), adjust = 1/5) + 
        theme_minimal() +
        ...
        facet_wrap( ~ type.str + set, ncol = 3) + 
...
for (ix.name in names(model.results$coefs)) {
  ...
  my.km.train[[ix]] <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), 
                                      ...
                                      plot.title = 'Train set')$plot + km.name('Train', ix.alpha, ix.name)
  my.km.test[[ix]]  <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), 
                                      xdata.test[, xdata.ix], ydata.test, 
                                      plot.title = 'Test set')$plot + km.name('Test', ix.alpha, ix.name)
  ix <- ix + 1
}
...
if (params$train >= 1) {
  ...
  ntimes.results <- parallel::mclapply(seed.vec, function(seed) {
    ...
    #
    flog.info(sprintf('Resampling with seed: %d (%d)', seed, which(seed == seed.vec)))
    ...
    flog.info(sprintf('Finish with seed: %d (%d)', seed, which(seed == seed.vec)), my.output, capture = TRUE)
  ...
  rank.df$type <- factor(rank.df$type, levels = c('c.index.train', 'c.index.test', 'km.train', 'km.test'), 
                                                    labels = c('C-Index (Train set)', 'C-Index (Test set)', 'Log-rank (Train set)', 'Log-rank (Test set)'))
}
```
...
```{r c.index.iter, echo=FALSE, fig.height=12}
if (nrow(big.df) > 0) {
  as.tbl(big.df) %>%
    mutate(model.str = prettify.labels(model, 'string.short')) %>%
    mutate(model.name = prettify.labels(model, 'model')) %>%
    filter(metric %in% c('C-Index (Test set)')) %>%
    filter(values >= 0) %>%
    ggplot() +
      #geom_freqpoly(aes(values, color = model), alpha = .75, bins = 100) +
      stat_density(aes(values, fill = model.str), alpha = .75, adjust = 1/5) + 
      facet_wrap(model.str ~ metric, ncol = 3) +
      theme_minimal() + theme(legend.position = 'none') +
      ggtitle('C-Index for Test set')
}
```

#### Rank

```{r rank.c.index.test, warning=FALSE}
...
if (nrow(big.df) > 0) {
  as.tbl(big.df) %>%
    ...
    ggplot() +
      #geom_freqpoly(aes(values, color = model), alpha = .75, bins = 100) + 
      stat_density(aes(values, fill = model), alpha = .75, adjust = 1/5) + 
      facet_wrap(model ~ metric, ncol = 3) +
      theme_minimal() + theme(legend.position = 'none') +
      ggtitle('C-index for Train set')
}
```

#### Rank

```{r rank.c.index.train, warning=FALSE}
...
```{r rank.log.rank.train, warning=FALSE}
...
if (nrow(big.df) > 0) {
  ...
  for (type in unique(all.origin)) {
    ...
    tryCatch({
      draw.3venn(ll, prettify.labels(type, 'string.base'))
      #vv <- Venn(ll)
      #gridExtra::grid.arrange(grid::grid.grabExpr(plot(vv, doWeights = TRUE, type = 'circles')), top = prettify.labels(type, 'string.base'))
      }, error = function(err) { flog.error('Probel with %s', err)})
  }
}
```
